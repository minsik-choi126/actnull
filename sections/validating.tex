...
$-0.000623$ at $s=0$. Both arms score the same planted pairs and differ only in the randomisation.
...
To expose how existing baselines behave on the same reference pairs, we form semi-synthetic pairs by
independently applying our activation-conditioned generator to real updates. Residual orientation
...
own surrogate mean. Thus entries share overlap units but not an estimand. Our row is circular and
is shown only as an implementation check.}
\label{tab:baselines}
\begin{tabular}{@{}lr@{}}
\toprule
baseline & reported excess \\
\midrule
exact orthogonality~\citep{gargiulo2025task}        & $+0.0275$ \\
...
For the activation null, delivered-signal recovery rises from $45.6$--$51.7\%$ to $99.3$--$99.6\%$
while the unplanted offset moves from $+0.00004$ to $-0.000623$. This is the desired combination:
small reference offset and high response to the planted alternative. The study covers four
modules and one family of interventions, and planting can alter conditioned occupancies, so we do
not transfer the sub-percent deviation as a correction or bound on the $72$-module
