...
def classifier_heads(model, tokenizer, names: list[str], device: str) -> torch.Tensor:
    with torch.no_grad():
        embeddings = []
        for name in names:
...
def evaluate(model, processor, tokenizer, test_sets, device: str, batch: int):
    ...
    for task, (dataset, image, label, names) in test_sets.items():
        heads = classifier_heads(model, tokenizer, names, device)
        correct = total = 0
        with torch.no_grad():
        ...
        print(f"    {task:14s} {accuracies[task]:5.2f}% (n={total})", flush=True)
    return accuracies
...
def main() -> None:
    ...
    def apply_projection(kind: str, rank: int) -> None:
        for module, values in deltas.items():
            ...
            state[module + ".weight"].copy_(
                original[module] + args.lambda_scale * update
            )
