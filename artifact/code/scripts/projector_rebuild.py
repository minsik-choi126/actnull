#!/usr/bin/env python3
"""Measure projector-basis overlap with an activation-conditioned null rebuild.

The script forms a cumulative update, extracts its top-r_p right singular
subspace, rebuilds the cumulative update after independently applying the
arm-appropriate conditional null to each task vector, and measures the two
projector bases' overlap. The resulting value is a null-reproduced baseline
diagnostic: it quantifies basis stability under the specified randomisation but
does not label components as learned or unlearned, establish causality, or assess
the downstream merging method's accuracy.

Usage:
  python scripts/projector_rebuild.py --cov <dir> --base <ckpt> --key_prefix vision_model. \
...
def main():
    ...
    for mi, mod in enumerate(mods):
        ...
        # The corresponding projector rebuilt under independent conditional-null draws.
        ov = []
    ...
    print("\n  This is a conditional-null reproduction diagnostic, not a causal"
          "\n  attribution or an evaluation of downstream merging accuracy.")
