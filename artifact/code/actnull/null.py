#!/usr/bin/env python3
"""Conditional read-subspace overlap nulls used by the PRISM v7 artifact.

The primary activation-block null conditions on each observed update's spectrum
and calibrated activation-coordinate structure, then applies sign-corrected Haar
randomisation within unresolved eigenvalue blocks and the full orthogonal
complement. The shared-A LoRA null conditions on the adapter A factor and the
observed spectrum of B while Haar-randomising B's singular frames. Exact overlap
scripts analytically integrate scalar and block moments where applicable.

The reported statistic compares observed top-k right-subspace overlap with its
arm-specific conditional-null level. Activation whitening and the generative
activation model are separate sensitivity and baseline diagnostics. Independently
generated, observed-spectrum-matched H0 pairs provide an external level stress
diagnostic; their value is reported separately and is never subtracted from the
observed-data estimand.

The command-line entry point consumes covariance tensors and immutable checkpoint
snapshots. The compact release ships numerical outputs, provenance records, and
an independent checker; large tensors and model weights remain upstream inputs.
...
def gaussian_haar_frame(x: torch.Tensor) -> torch.Tensor:
    """Return the Haar/Stiefel frame obtained from a real Gaussian matrix.

    A bare QR factor is not Haar: the implementation's convention for the signs of
    ``diag(R)`` selects one representative from each column-sign equivalence class. Folding
    those signs back into Q makes the diagonal of R nonnegative and restores the uniform
    orthogonal/Stiefel law. The final dimension indexes frame columns, so this also supports
    batched inputs.
    """
...
def act_matched_null(dW: torch.Tensor, w: torch.Tensor, V: torch.Tensor,
    ...
    RETAINED FOR COMPARISON, NOT AS THE PRIMARY NULL. It assumes a one-step relationship
    between update covariance and C_H with an isotropic output side. The common-reference
    diagnostic archives the resulting level failure separately. An earlier exploratory
    implementation omitted the unresolved complement; its quantitative output is invalid and
    must not be reused. The conditional null instead preserves each observed update's occupancy
    profile rather than generating that relationship from this unverified model."""
    d_out, d_in = dW.shape
    m = V.shape[0]
    Vd = V.to(dW)
...
def h0_pair(spectrum: torch.Tensor, w: torch.Tensor, V: torch.Tensor, trace_full: float,
            d_out: int, gen):
    """Build one sample for the external activation-generated H0 stress diagnostic.

    The generator matches an observed singular-value spectrum and the base activation
    geometry, then supplies independent output-side randomness. It is deliberately external
    to the conditional randomisation orbit used by :func:`coupling_destroying_null`: an H0 pair
    is therefore useful for detecting conspicuous level failures under this synthetic model,
    but it is neither a same-Haar self-null nor an estimate of an additive bias. Its measured
    excess is reported separately and is never subtracted from the observed-data estimand.
    """
    d_in = V.shape[1]
    m = V.shape[0]
...
def coupling_destroying_null(dW: torch.Tensor, V: torch.Tensor, n_blocks: int, gen,
    ...
    for blk in blocks:
        ...
        if n < 2:
            ...
            C[:, idx] = C[:, idx] * sgn
            continue
        Q = gaussian_haar_frame(torch.randn(n, n, generator=gen,
    ...
    # cross-task agreement it was supposed to destroy. On controlled pairs with 2 of 8 planted
    # read directions, it reproduced 41% (d_in=768) to 70% (d_in=3072) of that signal in the
    # null level. The H0 check cannot
    ...
    if rank >= 2 and float(R.norm()) > 0:
        ...
        G = gaussian_haar_frame(G)                       # uniform frame of the complement
        R = (M @ G.T).to(dW)
    return C @ Vd + R
    ...
    def __init__(self, path: str):
        ...
        def resolve_file(name: str) -> Path:
            if local.is_dir():
                ...
                return candidate
            return Path(hf_hub_download(path, name))

        cfg = json.loads(resolve_file("adapter_config.json").read_text())
        ...
        for cand in ("adapter_model.safetensors", "linearized_adapter_model.safetensors"):
            try:
                f = resolve_file(cand); self.kind = cand; break
...
def lora_shared_init_null(A: torch.Tensor, B: torch.Tensor, scale: float, gen):
    """Factor-level conditional null for a shared-A LoRA pool.

    For dW = (alpha/r) B A, the row space is constrained to row(A), and equals it whenever B
    has full column rank. The null therefore keeps the observed A factor and the singular
    values of B fixed, while independently Haar-randomising B's left and right singular
    frames. It estimates the overlap level reproduced by these conditioned factors without
    treating that level as learned or unlearned structure.
    ...
    excess:
      ...
      own spectrum in both adapters. Two true-null draws overlap at 0.706, not
    ...
    U = gaussian_haar_frame(torch.randn(B.shape[0], r, generator=gen,
                                        device=B.device, dtype=torch.float32))
    W = gaussian_haar_frame(torch.randn(B.shape[1], r, generator=gen,
                                        device=B.device, dtype=torch.float32))
    G = (U * Sb) @ W.T
    return (G @ A) * scale
...
def main():
    ...
    ap.add_argument("--h0_draws", type=int, default=8,
                    help="external activation-generated, observed-spectrum-matched H0 pairs "
                         "PER MODULE. Their excess is a stress diagnostic reported in "
                         "h0_excess; it is not a same-Haar self-null and is never subtracted "
                         "from the observed-data estimand. 0 disables the diagnostic.")
    ...
    for mi, mod in enumerate(modules):
        ...
        # Per-module truncation. The participation ratio (sum w)^2 / sum w^2 is used here as
        # an operational effective-rank summary of activation anisotropy, not as a formal
        # identifiability cutoff for individual eigenvectors.
        ...
        # ---- external H0 stress diagnostic (reported separately, never subtracted) --------
        h0_ex = 0.0
        if args.null == "lora":
            # The LoRA null already redraws B conditionally on A, so this activation-generated
            # diagnostic is not applicable to it.
            pass
        ...
        for i, j in itertools.combinations(live, 2):
            ...
            rows.append(dict(module=mod, d_in=d_in, a=i, b=j, k=args.k, m=m_mod,
                             ...
                             h0_excess=h0_ex,
                             # Backward-compatible legacy column. H0 is not an additive
                             # correction, so this is now an alias of the primary estimand.
                             excess_raw_corrected=r_ - null_raw))
        if (mi + 1) % 20 == 0:
    ...
    print("  `h0_excess` is an external stress diagnostic and is not subtracted. The legacy")
    print("  `excess_raw_corrected` column is retained as an alias of `excess_raw`.")
